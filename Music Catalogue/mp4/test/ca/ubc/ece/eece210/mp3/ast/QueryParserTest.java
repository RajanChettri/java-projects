...
public class QueryParserTest {
    ...
    public void testIn() {
        ...
        assertEquals( "Jazz", rootNode.arguments ); // Root node is Jazz
    }
    ...
    public void testAnd() {
        ...
        assertEquals( null, root.arguments );
    }
    ...
    public void testBy() {
        List<Token> tokens = QueryTokenizer
                .tokenizeInput( "by (\"Louis Armstrong\")" );
        QueryParser parser = new QueryParser( tokens );
        ASTNode root = parser.getRoot(); // get the root node
        assertEquals( "by", root.toString() ); // name of root note should be by
        assertEquals( "Louis Armstrong", root.arguments );
    }
}